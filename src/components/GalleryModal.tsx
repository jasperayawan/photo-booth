  ...
  return (
    ...
    >
      <div className="relative bg-black max-w-2xl w-full flex justify-center items-center mx-auto flex-col gap-y-2 p-5 rounded-md">
        ...
        <div
          className={`flex flex-col w-[max-content] justify-center items-center relative group rounded-[2px] ${
            currentPhoto?.frame.name === "None"
              ? "bg-white px-4 pt-2"
              : currentPhoto?.frame.style
          }`}
          style={{ backgroundColor: currentPhoto.frame.strokeColor }}
        >
          <img
          ...
          />
          <span className={`w-full text-center random-words py-5`}>
            {currentPhoto?.loveWord}
          </span>
        </div>
        <button
